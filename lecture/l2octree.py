...
class Octant:
    def __init__(self, children, center, extent, point_indices, is_leaf):
        ...
        self.extent = extent  # 0.5 * length, 每次划分的格子大小。这里面下一级格子都是上一级格子的一半。
...
def octree_knn_search(root: Octant, db: np.ndarray, result_set: KNNResultSet, query: np.ndarray):
    ...
    :param root:
    :param db: point data
    :param result_set: Nearest point set
    :param query: target point
    :return:
    """
    if root is None:
    ...
    if root.is_leaf and len(root.point_indices) > 0:
        ...
        return inside(query, result_set.worstDist(), root)  # if query ball is inside an octant, stop
